...
export const CarbonFootprint = () => {
        ...
        try {
            ...
            if (!userId) {
                if (isFirstTime) {
                    ...
                    const { data } = await axios.post('https://api.pureprakruti.com/api/vehicle/getCarbonFootPrints', formData);
    ...
    return (
        <div
            className="w-full min-h-screen overflow-auto items-center justify-center bg-cover bg-center bg-opacity-40 relative"
        >
            <img className="absolute top-0 left-0 w-full h-full object-cover z-0" src="/bglogin3.jpg" alt="Background image" />
            <div className='relative px-4 sm:px-6'>
                <div>
                    <h1 className="mt-28 2xl:mt-40 xl:mt-36 lg:mt-32 sm:mt-24 text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-2 text-gray-200">Carbon Footprint Calculator</h1>
                    <div className="flex justify-center mb-4">
                        <motion.div
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.8, delay: 0.2 }}
                            className="flex items-center bg-white/10 backdrop-blur-md px-3 py-1 md:px-4 md:py-1.5 rounded-full border border-white/20 w-fit"
                        >
                            <span className="text-green-400 mr-2 text-lg">✨</span>
                            <span className="text-white text-sm md:text-base font-semibold tracking-wide font-['Outfit']">Powered by AI Analysis</span>
                        </motion.div>
                    </div>
                    <h2 className="pt-2 text-xl sm:text-2xl md:text-3xl text-center lg:mb-6 text-green-100">Calculate your vehicle's environmental impact and get certified emissions data</h2>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-2">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 lg:gap-12 text-gray-100">
                                {['VechileNumber', 'SourcePincode', 'DestinationPincode', 'MobilizationDistance', 'DemobilizationDistance', 'LoadedWeight'].map((field) => (
                                    <div key={field}>
                                        <label className="block text-lg sm:text-xl font-medium mb-2" htmlFor={field}>
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 sm:px-6 py-2 sm:py-3 border text-lg sm:text-xl border-white/60 rounded p-2 text-white placeholder-white/80 bg-green-700/50 backdrop-blur-md focus:outline-none focus:ring-1 focus:ring-green-400"
